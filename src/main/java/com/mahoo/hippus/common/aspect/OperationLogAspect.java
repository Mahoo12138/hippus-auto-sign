package com.mahoo.hippus.common.aspect;
...
public class OperationLogAspect {
     ...
     */
    @Pointcut("@annotation(com.mahoo.hippus.common.annotation.OperLog)")
     ...
     */
    @Pointcut("execution(* com.mahoo.hippus.controller..*.*(..))")
    ...
    public void saveOperLog(JoinPoint joinPoint, Object keys) {
        ...
        try {
//            operationlog.setOperId(UuidUtil.get32UUID()); // 主键ID
            ...
            if (opLog != null) {
                String oModel = opLog.operModule();
                String oType = opLog.operType();
                String oDesc = opLog.operDescription();
                operationlog.setModule(oModel); // 操作模块
                operationlog.setType(oType); // 操作类型
                operationlog.setDescription(oDesc); // 操作描述
            }
            // 获取请求的类名
...
//            operationlog.setUserId(UserShiroUtil.getCurrentUserLoginName()); // 请求用户ID
//            operationlog.setUserName(UserShiroUtil.getCurrentUserName()); // 请求用户名称
//            operationlog.setIp(IPUtil.getRemortIP(request)); // 请求IP
            operationlog.setUrl(request.getRequestURI()); // 请求URI
            operationlog.setCreateTime(new Date()); // 创建时间
            operationlog.setVersion(version); // 操作版本
...
//            excepLog.setIp(IPUtil.getRemoteIP(request)); // 操作员IP
            excepLog.setVersion(version); // 操作版本号
            excepLog.setCreateTime(new Date()); // 发生异常时间
            log.error("{}", JSONUtil.parse(excepLog));
    ...
    public String stackTraceToString(String exceptionName, String exceptionMessage, StackTraceElement[] elements) {
        ...
        for (StackTraceElement stet : elements) {
            struct.append(stet + "n");
        }
        String message = exceptionName + ":" + exceptionMessage + "nt" + struct.toString();
        return message;
    }
}
